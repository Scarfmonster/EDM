...
class Envelope:
    ...
    def __str__(self):
        res = [f'{x:.3f}'.rstrip('0').rstrip('.')
               for x in [*self.p[:3], *self.v[:4]]]
        if len(self.p) >= 4:
            res.extend(['%', f'{self.p[3]:.3f}'.rstrip('0').rstrip('.')])
        if len(self.p) == 5:
            res.extend([f'{x:.3f}'.rstrip('0').rstrip('.')
                       for x in [self.p[4], self.v[4]]])
        return ','.join(res)
...
class Mode1Pitch:
    def __init__(self, PBStart='', PitchBend=''):
        ...
        self.pitches = [
            float(x) if x != '' else 0 for x in PitchBend.split(',')]
...
class Vibrato:
    ...
    def __str__(self):
        tmp = [self.length, self.cycle, self.depth,
               self.fade_in, self.fade_out, self.phase, self.offset]
...
class Note:
    ...
    def set_preutterance(self, preutterance):
        ...
        self.note_data['PreUtterance'] = f'{preutterance:.3f}'.rstrip(
            '0').rstrip('.')
    ...
    def set_overlap(self, overlap):
        self.note_data['VoiceOverlap'] = f'{overlap:.3f}'.rstrip(
            '0').rstrip('.')
    ...
    def set_intensity(self, intensity):
        self.note_data['Intensity'] = f'{intensity:.3f}'.rstrip(
            '0').rstrip('.')
    ...
    def set_modulation(self, modulation):
        self.note_data['Modulation'] = f'{modulation:.3f}'.rstrip(
            '0').rstrip('.')
    ...
    def set_start_point(self, start_point):
        self.note_data['StartPoint'] = f'{start_point:.3f}'.rstrip(
            '0').rstrip('.')
...
class UtauPlugin:
    ...
    def new_empty(cls):
        ...
        self.version = 'UST Version1.2'
        self.settings = {'Tempo': '100.00', 'Tracks': '1', 'ProjectName': '', 'VoiceDir': '',
                         'OutFile': '', 'CacheDir': '', 'Tool1': '', 'Tool2': '', 'Mode2': 'True'}
        self.prev_note = None
        self.next_note = None
        self.notes = []
